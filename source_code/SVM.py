...
tweets = []
for line in open('TWITTER.txt').readlines():
...
print clf.predict(X)
tweets = []
for line in open('TESTINGTWITTER.txt').readlines():
    tweets.append(json.loads(line))
...
print '\r\nOut of total {1} tweets in tweets.txt file, {0} are predicted as positive.'.format(sum(y == 1), len(y))

print '\r\nOut of the total {1} tweets in tweets.txt file, {0} are predicted as negative.'.format(sum(y == 0), len(y))

print '\r\nOut of the total {1} tweets in tweets.txt file, {0} are predicted as advertisements.'.format(sum(y == 2),
                                                                                                        len(y))